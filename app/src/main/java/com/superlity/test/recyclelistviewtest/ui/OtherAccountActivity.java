    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.chat_with_other:
                ...
                startConversition(otherId);
                CCPSDKCoreHelper.getInstance().setOtherId(otherId);
                break;
        }
    }

    private void startConversition(String otherId) {
        if (!TextUtils.isEmpty(otherId)) {
            ChatManager.getInstance().fetchConversationWithUserId(otherId, new AVIMConversationCreatedCallback() {
                @Override
                public void done(AVIMConversation conversation, AVIMException e) {
                    if (e != null) {
                        ...
                        //这里先注册一下这个会话
                        ChatManager.getInstance().registerConversation(conversation);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case android.R.id.home:
                Intent intent = new Intent(OtherAccountActivity.this, LoginActivity.class);
                startActivity(intent);
                finish();
                break;
        }
